...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch('http://117.72.172.224:8090/api/v1/rag/query_rag_tag_list')
...
function startEventStream(message) {
    ...
    if (ragTag) {
        url = `http://117.72.172.224:8090/api/v1/${aiModelValue}/generate_stream_rag?message=${encodeURIComponent(message)}&ragTag=${encodeURIComponent(ragTag)}&model=${encodeURIComponent(aiModelModel)}`;
    } else {
        url = `http://117.72.172.224:8090/api/v1/${aiModelValue}/generate_stream?message=${encodeURIComponent(message)}&model=${encodeURIComponent(aiModelModel)}`;
    }
